...
over /Applications/Bitcoin-Qt (on Mac) or megacoind/megacoin-qt (on Linux).
...
TX outputs, bloating megacoin's UTXO database.
...
megacoind makes it easier for experienced open source developers to contribute 
to the project.
...
Another change in the 0.9 release is moving away from the megacoind executable
functioning both as a server and as a RPC client. The RPC client functionality
("tell the running megacoin daemon to do THIS") was split into a separate
executable, 'megacoin-cli'. The RPC client code will eventually be removed from
megacoind, but will be kept for backwards compatibility for a release or two.
...
- Add '-disablewallet' mode to let megacoind run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with megacoind
...
- Added new DNS seed from megacoinstats.com
...
- Show and store message of normal megacoin:URI
- Fix richtext detection hang issue on very old Qt versions
- OS X: Make use of the 10.8+ user notification center to display Growl-like 
  notifications
- OS X: Added NSHighResolutionCapable flag to Info.plist for better font
  rendering on Retina displays.
- OS X: Fix megacoin-qt startup crash when clicking dock icon
- Linux: Fix Gnome megacoin: URI handler
...
- Add separate megacoin-cli client
