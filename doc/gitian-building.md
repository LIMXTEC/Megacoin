...
*Setup instructions for a Gitian build of MegaCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MegaCoin
...
- [Building MegaCoin Core](#building-megacoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-megacoin-host/README.md).
...
Clone the git repositories for megacoin and Gitian.
...
in the megacoin repository under 'Fetch and create inputs' to install sources which require
...
To build MegaCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the megacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/megacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/LIMXTEC/megacoin
...
For example:
```bash
URL=https://github.com/thrasher-/megacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit megacoin=${COMMIT} --url megacoin=${URL} ../megacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit megacoin=${COMMIT} --url megacoin=${URL} ../megacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit megacoin=${COMMIT} --url megacoin=${URL} ../megacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the megacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../megacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/megacoin
SIGPATH=/some/root/path/megacoin-detached-sigs

./bin/gbuild --url megacoin=${BTCPATH},signature=${SIGPATH} ../megacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/megacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/megacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/megacoin-osx-build.assert
```
...
[megacoin-project/gitian.sigs.mec](https://github.com/megacoin-project/gitian.sigs.mec/) repository, or if that's not possible create a pull
