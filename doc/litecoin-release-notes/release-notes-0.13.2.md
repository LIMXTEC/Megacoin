MegaCoin Core version 0.13.2 is now available from:

  <https://download.megacoin.cc/megacoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a megacoin
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Bitcoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/megacoin/megacoin/issues/7681#issuecomment-217439891)
...
ECDSA signatures inside MegaCoin transactions now use validation using
[libsecp256k1](https://github.com/megacoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/megacoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of MegaCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of MegaCoin Core was to raise the default minimum
relay fee.

MegaCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
MegaCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running megacoind without having to do any manual configuration.
...
MegaCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. MegaCoin Core will relay transactions with
...
entirely in MegaCoin Core 0.13, and it is at this time undecided whether the
...
MegaCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
MegaCoin Core automatically creates a hidden service to listen on, without
manual configuration. MegaCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if MegaCoin Core is listening, and
...
MegaCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals MegaCoin
Core to use floating fees. By default, MegaCoin Core will use floating
fees.
...
(default: `0.0002` MEC/kB).

At all times, MegaCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MEC.
Furthermore, MegaCoin Core will never create transactions paying less than
...
`megacoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of Bitcoin Core v0.11.0](https://github.com/megacoin/megacoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/megacoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/megacoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /megacoinrpc>
        ...
        # optional bypass megacoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< megacoinrpc:<password>
    </Location>
...
Restarting megacoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in megacoin.conf
...
megacoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/megacoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The MegaCoin Core code base has
...
- `megacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `megacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/megacoin/megacoin/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the MegaCoin scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/megacoin/megacoin/pull/7524>
...
MegaCoin transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/megacoin/megacoin/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/megacoin/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of MegaCoin Core.

[Pull request](https://github.com/megacoin/megacoin/pull/8035/files), [BIP 32](https://github.com/megacoin/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/megacoin/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/megacoin/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the MegaCoin-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/megacoin/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/megacoin/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/megacoin/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/megacoin/megacoin/pull/7762).
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming MegaCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for MegaCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each MegaCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of megacoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of megacoins being spent is exactly
...
- **Increased security for multisig:** MegaCoin addresses (both P2PKH addresses
  ...
  believed to be provided by MegaCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of MegaCoin's current ledger (called the UTXO set),
  ...
  note that MegaCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  MegaCoin users to individually opt-in to almost any change in the MegaCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and MegaCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the MegaCoin
client to MegaCoin Core 0.13.2 to signal for segwit support. When a
...
[Segwit FAQ]: https://megacoincore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://megacoincore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/megacoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/megacoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/megacoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/megacoin/bips/blob/master/bip-0145.mediawiki
...
Since MegaCoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/megacoin/bips/blob/master/bip-0147.mediawiki
...
- `megacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `megacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
