...
###   megacoincore.org and megacoin.cc and compares them.
...
WORKINGDIR="/tmp/megacoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://megacoincore.org"
HOST2="https://megacoin.cc"
BASEDIR="/bin/"
VERSIONPREFIX="megacoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'megacoin-',
...
if [ $? -ne 0 ]; then
   echo "megacoin.cc failed to provide signature file, but megacoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "megacoin.cc and megacoincore.org signature files were not equal?"
