...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if ((nVersion < sporkManager.GetSporkValue(SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION))&& (sporkManager.GetSporkValue(SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION) < PROTOCOL_VERSION))
        {
            // disconnect from peers older than this proto version
            LogPrint(BCLog::NET, "SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION peer=%d using obsolete version %i; disconnecting\n", pfrom->GetId(), nVersion);
            if (enable_bip61) {
                connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                                   strprintf("SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION Version must be %d or greater", MIN_PEER_PROTO_VERSION)));
                // FXTC BEGIN
                // obsolete peers are very annoying
                connman->Ban(pfrom->addr, BanReasonNodeMisbehaving);
                // FXTC END
        ...
        {
            LogPrint(BCLog::NET,"SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION Disable!\n");
        }
        LogPrint(BCLog::NET, "SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION = %d\n", sporkManager.GetSporkValue(SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION));
        
        if (sporkManager.GetSporkValue(SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION) < PROTOCOL_VERSION)
        LogPrint(BCLog::NET, "SPORK_MEGACOIN_16_MIN_PEER_PROTO_VERSION < PROTOCOL_VERSION \n");
        */
        // MEC END
