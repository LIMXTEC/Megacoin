...
template<typename T1>
inline uint256 Mega_Mec_2(const T1 pbegin, const T1 pend,uint32_t timestamp)
{
    sph_blake512_context     ctx_blake;
    sph_bmw512_context       ctx_bmw;
    sph_groestl512_context   ctx_groestl;
	...
	uint32_t permutation_3[HASH_FUNC_COUNT_3 + HASH_FUNC_COUNT_2 + HASH_FUNC_COUNT_1];
			...
			for (int i = HASH_FUNC_COUNT_2; i < HASH_FUNC_COUNT_1 + HASH_FUNC_COUNT_2 + HASH_FUNC_COUNT_3; i++) {
                ...
                LYRA2(static_cast<void*>(&hash[22]), 32, static_cast<const void*>(&hash[23]), 32, static_cast<const void*>(&hash[23]), 32, 1, 4, 4);
return ArithToUint256(hash[23].trim256());
}
