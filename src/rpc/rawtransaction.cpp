...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry, bool expanded = false)
{
    ...
    // available to code in megacoin-common, so we query them here and push the
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the megacoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
