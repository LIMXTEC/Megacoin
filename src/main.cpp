...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Check proof of work
	if(nHeight >= 62800){  // Limxdev skip the function FreeCoin Diffalgo 
    if ((!Params().SkipProofOfWorkCheck()) &&
       (block.nBits != GetNextWorkRequired(pindexPrev, &block)))
        return state.DoS(100, error("%s : incorrect proof of work", __func__),
                         REJECT_INVALID, "bad-diffbits");
						 } // LimxDev Disable Freecoin Diff Algo
    ...
    // Megacoin: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Megacoin: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
